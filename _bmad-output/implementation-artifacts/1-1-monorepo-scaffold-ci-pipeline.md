...
_Generated by `bmad-code-review` — 3 reviewer layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor) over 49 raw findings. After dedup + triage: 3 decision-needed → 25 patches applied + 1 dismissed (false positive), 14 deferred (P15 + 13 originals), 6 originally dismissed._

**Decision-needed (resolved):**

- [x] [Review][Decision] **Web UI customizations bypass Story 1.3 / Epic 3** — **DISMISSED as false positive after course correction.** Diff'd every web file against a freshly bootstrapped `npm create vite@latest -- --template react-ts` (`create-vite@9.0.4`): byte-identical. The Acceptance Auditor and review LLM both worked from outdated knowledge of Vite's defaults — the current template ships a 121-line App.tsx with hero image, social links, and `#aa3bff` accent as its default content. Nothing was customized. Real fix preserved: D3 (`rel="noopener noreferrer"`) was promoted from this dismissal to a standalone PATCH and applied.
- [x] [Review][Decision] **React minor version mismatch web 19.2.4 vs mobile 19.1.0** — **RESOLVED → PATCH applied:** mobile bumped to `^19.2.0` + `@types/react ~19.2.0`. Both apps now align on React 19.2.x.
- [x] [Review][Decision] **External links in `apps/web/src/App.tsx` lack `rel="noopener noreferrer"`** — **RESOLVED → PATCH applied:** all 6 `target="_blank"` anchors now include `rel="noopener noreferrer"`. (Kept as a separate patch instead of folding into D1 since D1 turned out to be a false positive.)

**Patch (24 applied + 1 deferred — see Change Log entry 9):**

- [x] [Review][Patch] Typo `CC#` corrupting H1 heading [`_bmad-output/planning-artifacts/implementation-readiness-report-2026-04-15.md:16`]
- [x] [Review][Patch] Remove dead `apps/api/docs/.gitkeep` (directory now contains real generated files)
- [x] [Review][Patch] Change Log entry 8 omits Vite 8 — extend to mention Vite version bump
- [x] [Review][Patch] `apps/api/package.json:typecheck` duplicates `lint` (both `go vet`) — changed to `go build -o /dev/null ./...`; `test` also gained `-timeout 5m`
- [x] [Review][Patch] `turbo.json:build.outputs` includes dead `.next/**` paths from Turborepo starter — removed
- [x] [Review][Patch] `.env.example` files pre-list `CLERK_*` and `STRIPE_*` keys despite both being explicit Out-of-Scope — stripped from all 3 .env.examples
- [x] [Review][Patch] Pin `swag` CLI version in CI to match `go.mod` (`@v2.0.0-rc5`) instead of `@latest` [`.github/workflows/ci.yml`]
- [x] [Review][Patch] Replace `go-version: "stable"` with `go-version-file: apps/api/go.mod` [`.github/workflows/ci.yml`]
- [x] [Review][Patch] Use `pnpm exec openapi-typescript` instead of bare invocation in `generate:types` [`scripts/generate-types.sh`]
- [x] [Review][Patch] Add `concurrency: { group: ci-${{ github.ref }}, cancel-in-progress: true }` to CI workflow [`.github/workflows/ci.yml`]
- [x] [Review][Patch] CI sync check also fails on new untracked generated files via `git status --porcelain` check [`.github/workflows/ci.yml`]
- [x] [Review][Patch] Pin `pnpm/action-setup@v4` with `version: 10.33.0` (matches root `packageManager`) [`.github/workflows/ci.yml`]
- [x] [Review][Patch] `go test -timeout 5m` in CI [`.github/workflows/ci.yml`]
- [x] [Review][Patch] Added `permissions: { contents: read }` block at workflow level [`.github/workflows/ci.yml`]
- [ ] [Review][Patch] **DEFERRED** Move `github.com/swaggo/swag/v2` import to `tools.go` with `//go:build tools` build tag — proper fix requires postprocessing the regenerated `docs.go` and is more invasive than initially scoped. Moved to `deferred-work.md` as a focused tooling story.
- [x] [Review][Patch] Delete empty `.npmrc` (no useful pnpm setting needed yet)
- [x] [Review][Patch] Web `eslint.config.js`: bumped `ecmaVersion` 2020 → 2022; added `node_modules` to `globalIgnores` [`apps/web/eslint.config.js`]
- [x] [Review][Patch] Replaced non-null assertion on `getElementById('root')` with explicit guard + descriptive error [`apps/web/src/main.tsx`]
- [x] [Review][Patch] `main_test.go`: added `t.Cleanup` to restore prior gin mode [`apps/api/cmd/server/main_test.go`]
- [x] [Review][Patch] `turbo.json`: `dev` task now `dependsOn: ["^generate:types"]` [`turbo.json`]
- [x] [Review][Patch] `turbo.json`: `generate:types` task gained `inputs` (apps/api/cmd/**, apps/api/internal/**) + `outputs` (apps/api/docs/**, packages/types/generated/**) [`turbo.json`]
- [x] [Review][Patch] `generate:types` extracted to `scripts/generate-types.sh`: validates `$(go env GOPATH)/bin/swag` exists; checks swagger.json non-empty; uses subshells for cwd safety
- [x] [Review][Patch] (Same as above — script-extraction handled subshell concern)
- [x] [Review][Patch] `apps/mobile/eslint.config.js`: `ignores` extended with `ios/*`, `android/*`, `expo-env.d.ts` [`apps/mobile/eslint.config.js`]
- [x] [Review][Patch] **D2/P26**: bumped mobile `react` to `^19.2.0` and `@types/react` to `~19.2.0` to align with web 19.2.x [`apps/mobile/package.json`]
- [x] [Review][Patch] **D3/new**: added `rel="noopener noreferrer"` to all 6 `target="_blank"` anchors [`apps/web/src/App.tsx`]

**Deferred (real but out of Story 1.1 scope — see `deferred-work.md`):**

- [x] [Review][Defer] Gin `Run()` lacks read/write/idle timeouts and graceful shutdown handling — defer to Story 1.2 or 1.4 (production hardening, not scaffold scope)
- [x] [Review][Defer] `PORT` env not validated (non-numeric / out-of-range / injection) — defer to Story 1.2 or 1.4
- [x] [Review][Defer] No EADDRINUSE-distinct error message when port is bound — defer to Story 1.2
- [x] [Review][Defer] Gin not switched to `ReleaseMode` in production — defer to Story 1.4
- [x] [Review][Defer] Generated `apps/api/docs/docs.go` never imported (no `/swagger/*` route served) — defer to Story 1.4 when API endpoints meaningfully exist
- [x] [Review][Defer] Root `.gitignore` vs `apps/mobile/.gitignore` rule inconsistency (`.env*` vs `.env*.local`) — defer
- [x] [Review][Defer] Windows shell incompatibility in `generate:types` script — defer (Mac/Linux is current dev env)
- [x] [Review][Defer] ESLint version drift between workspaces; should adopt pnpm `catalog:` — defer (not felt yet)
- [x] [Review][Defer] `go.work` declares `go 1.26.2` but contributors with older Go get cryptic error — defer (README mentions Go 1.22+ requirement)
- [x] [Review][Defer] `slog` log level not configurable via `LOG_LEVEL`/`DEBUG` env — defer to Story 1.4 (observability scope)
- [x] [Review][Defer] `go run` child process not killed when `pnpm dev` is Ctrl-C'd — defer to a tooling improvement story (already observed once during AC2 verification; standard Turborepo limitation)
- [x] [Review][Defer] Test does not validate response `Content-Type` before `json.Unmarshal` — defer (overengineering for a smoke test)
- [x] [Review][Defer] `PORT=0` (ephemeral port) handling — defer

**Dismissed (false positives or already accepted):**

- Acceptance Auditor flagged `apps/api/go.sum` and `pnpm-lock.yaml` as missing from diff — both are present in `HEAD~3..HEAD` but were excluded by the reviewer's diff filter. False positive.
- Web/mobile `test` scripts hard-coded as `echo "no tests" && exit 0` — intentional per Resolved Decision #5.
- Broader React/Expo/TS/Go/pnpm version drift — pre-accepted in Change Log entries 1, 2, 5, 8.
- `.gitignore` `build`/`dist` patterns hiding `packages/types/generated/` — bare patterns don't match nested paths under `packages/`. False positive.
- pnpm `--frozen-lockfile` fails on lockfile divergence in CI — working as intended (the whole point of `--frozen-lockfile`).
...
**Created (root):**
- `package.json` (modified in review pass: `generate:types` script swapped to `bash scripts/generate-types.sh`)
- `pnpm-workspace.yaml`
- `turbo.json` (modified in review pass: removed `.next/**` outputs; `dev` task now `dependsOn: ["^generate:types"]`; `generate:types` task gained inputs/outputs)
- `.gitignore`
- `go.work`
- `pnpm-lock.yaml` (pnpm-managed)
- `README.md` (overwrote 2-line placeholder)
- `scripts/generate-types.sh` (added in review pass: extracted from inline npm script)

**Created (`.github/`):**
- `.github/workflows/ci.yml` (modified in review pass: workflow-level `concurrency` + `permissions`, `go-version-file`, pinned swag CLI to `@v2.0.0-rc5`, pinned `pnpm/action-setup` to `version: 10.33.0`, `go test -timeout 5m`, untracked-file detection in sync check)

**Deleted in review pass:**
- `.npmrc` (was empty; deleted as noise)
- `apps/api/docs/.gitkeep` (directory has real generated files now)
...
| 8 | 2026-04-16 | Vite template scaffolded **Vite 8.x + React 19.2 + TypeScript 6** (spec said Vite 5+ / React 18 / TS 5.x) | Template defaults moved on between spec date (2026-04-15) and scaffold date (2026-04-16). React 19 supports concurrent mode (TanStack Query v5 compat preserved); TS 6 is backward compatible for our patterns; Vite 8 raises minimum Node to ≥20.11 (we already pin that). |
| 9 | 2026-04-16 | **Code-review pass: 25 patches applied + 1 deferred** | `bmad-code-review` produced 49 raw findings → 3 decision-needed → 25 patches applied + 1 deferred (P15: swag dep isolation, requires postprocess on regenerated `docs.go`). One auditor finding (D1 "web UI customizations") was dismissed as a false positive after diff'ing against pristine `create-vite@9.0.4` — the alleged customizations are byte-identical to the current Vite template defaults. Mobile React aligned to web 19.2.x. CI workflow hardened (concurrency, permissions, version pins, untracked-file detection, test timeout). `generate:types` extracted to `scripts/generate-types.sh` with input validation. See `Review Findings` section for details. |
